...
# 停止：stop_all.ps1 会一并停止；或按端口 15055 / 进程名杀
...
$asrPort = if ($env:ASR_PORT) { [int]$env:ASR_PORT } else { 15055 }   # 2026-08-28：5055 被 Hyper-V 动态保留段 5050-5149 吞掉（WinError 10013），迁 15055
...
if ($asrListening -or $asrProc) {
    ...
    if (Test-Path $asrExe) {
        $env:ASR_PORT = "$asrPort"   # Start-Process 子进程继承父环境
