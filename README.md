...
- **指令感知（点名看图）**：打字或语音对桌宠说「看看我的屏幕」「我在干嘛」等指令，它会抓取前台窗口截图随你的消息一起发给视觉模型回答（因为你发出指令时焦点在桌宠身上，实际抓的是除桌宠外最靠前的窗口）。与自动感知相互独立——自动感知关着也能点名；共用「禁止抓取名单」，命中名单时不会截图，桌宠会口头说明该应用不可抓取。关键词可在控制页自定义（一行一个子串，含「别看/不要看」等否定词时不触发），截图失败（独占全屏/DRM/最小化等）也会如实说明。语音指令需要语音输入服务在线；打字指令始终可用。
