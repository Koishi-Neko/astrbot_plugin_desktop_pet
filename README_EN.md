...
- **Built-in Long-term Memory** (AstrBot mode): The pet remembers things on its own—no extra memory plugin needed. An LLM periodically reflects chats into memories, recalls them by vector similarity when relevant, and writes a little pet diary every night. You can also give direct orders: just say "remember xxx" (记住 xxx) or "remember forever" (永久记住 xxx). Memories have shelf lives—trivia fades in ~15 days, important things last ~120 days, and only "permanent" ones never fade. Covers three scopes: desktop pet, master's private QQ chats, and group chats—mixed together by default, but any scope can be set to "independent" on the control page (e.g. enable "group isolation" so private-chat memories never surface in group replies). Browse, edit, test recall, and view the memory graph on the control page; if you were using LivingMemory before, do a one-click full migration and then disable it.
...
| Long-term Memory | ✅ (Built into the plugin: vector recall + reflection + diary; pet/private/group scopes with optional isolation) | ❌ (Not in V1, sorry) |
