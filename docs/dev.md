...
- Live2D：`MODEL_CANDIDATES` 候选链（`config.local.json live2d.model_url` → chino → hiyori → 静态透明兜底）；`MODEL_PROFILES` 能力档案分流（hiyori 无 exp3 表情、用自带动作组、无 coin_sway）。pixi-live2d-display Cubism4，DPI 2x + mipmap。
